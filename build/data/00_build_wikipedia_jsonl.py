...
既定の入力:
  スクリプトと同じフォルダにある
  *-pages-articles.xml.bz2（なければ唯一の *.xml.bz2）
...
実行:
  py -3.13 00_build_wikipedia_jsonl.py
...
DEFAULT_OUTPUT = SCRIPT_DIR / "wikipedia_ja_from_dump.jsonl.bz2"
...
def find_default_input() -> Path:
    preferred = sorted(SCRIPT_DIR.glob("*-pages-articles.xml.bz2"))
    candidates = preferred or sorted(SCRIPT_DIR.glob("*.xml.bz2"))
...
def open_jsonl_output(path: Path, compressed: bool):
    if compressed:
        return bz2.open(
            path,
            "wt",
...
def main() -> int:
    args = parse_args()
    try:
        input_path: Path = args.input or find_default_input()
    ...
    try:
        with open_dump(input_path) as source, open_jsonl_output(
            partial_path,
            compressed=output_path.name.lower().endswith(".bz2"),
        ) as destination:
