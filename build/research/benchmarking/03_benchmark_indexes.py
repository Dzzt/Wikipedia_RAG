...
from raglib.faiss_helpers import create_index, train_index
...
def main() -> int:
    ...
    parser.add_argument("--vectors", type=Path, default=Path(r"sample\sample_vectors.f32"))
    parser.add_argument("--manifest", type=Path, default=Path(r"sample\sample_vectors.json"))
    parser.add_argument("--candidates", type=Path, default=Path(r"configs\candidates.json"))
    ...
    write_json(Path("configs") / "selected_config.json", selected)
